    ...
    ui <- fluidPage(navbarPage(title = h3('Predhy'),
          ...
          tabPanel(h2('Input genotype'),title='Genotype Input',
             fluidRow(column(width=4,fileInput('inbred_gen1',label = h4('inbred_gen'))),
                      ...
                      tabPanel(h2('Input phenotype'),title = 'Phenotype Input',
...
####################################cv##########################################    
  ...
  hybrid <- reactive({
    ...
    inbred <-as.data.frame(fread(input$hybrid_phe1$datapath,header = T,stringsAsFactors=F))
  })
  ...
  cvres <- reactive({
    if(input$cvfixinput=='input'){
      if(input$cvgendinput=='input'){
        cv(fix=fix(),gena = gena(),gend = gend(),y=y(),method=cvmethod(),nfold=nfold(),nTimes=ntimes(),seed=cvseed(),CPU=cpu())
      }else{
        cv(fix=fix(),gena = gena(),gend = NULL,y=y(),method=cvmethod(),nfold=nfold(),nTimes=ntimes(),seed=cvseed(),CPU=cpu())  
      }
    }else{
      if(input$cvgendinput=='input'){
        cv(fix=NULL,gena = gena(),gend = gend(),y=y(),method=cvmethod(),nfold=nfold(),nTimes=ntimes(),seed=cvseed(),CPU=cpu())
      }else{
        cv(fix=NULL,gena = gena(),gend = NULL,y=y(),method=cvmethod(),nfold=nfold(),nTimes=ntimes(),seed=cvseed(),CPU=cpu())
      }
    }
  })
  ...
  pred <- reactive({
    predhy.predict(inbred_gen(),hybrid_phe(),method(),model(),select(),number())
  })
